...
- LEAD (🟢) — коммерческие запросы/КП, заявки на работы
...
# Спамеры подменяют кириллические буквы похожими латинскими (C→С, A→А и т.д.)
...
GOV_KEYWORDS_SENDER = re.compile(
    ...
    r"прокуратур|пристав|инспекц|кадастров\w+\s*палат|госуслуг|"
    r"роспотребнадзор|ростехнадзор|росприроднадзор|пенсионн",
    re.IGNORECASE,
)
...
FINANCE_KEYWORDS = re.compile(
    ...
    r"упд\b|универсальн\w+\s*передаточн|"
    r"закрывающ\w+\s*документ|сверк\w+\s*(?:акт|взаиморасчёт|взаиморасчет)",
    re.IGNORECASE,
)
...
LEAD_KEYWORDS = re.compile(
    ...
    r"заявк\w+\s+на\s+(?:участ|выполнен|межеван|кадастр|геодез|"
    r"землеустро|техническ|топограф|вынос|разбивк|обследован|съёмк|съемк)|"
    r"(?:межев|кадастров|техническ|топографическ)\w+\s+"
    r"(?:план|работ|услуг|съёмк|съемк)|"
    r"техническ\w+\s+задани|"
    r"заказ\w*\s+(?:на\s+)?(?:межеван|кадастр|геодез|съёмк|съемк|вынос)",
    re.IGNORECASE,
)
...
def classify_email(email_data: dict) -> tuple[str, str]:
    ...
    # Нормализуем текст (латиница → кириллица для обхода подмены символов)
